...
understanding is such that Shor’s algorithm and its quantum computer implementation theoretically 
can efficiently factorize big number and solve discite logarithm problem. 
This algorithm, if powerful enough quantum computers will exist in the near future, 
puts the RSA and DH algorithms - the major building block of modern security 
...
$ed \equiv 1\ mod\ \phi(n)$ using modular multiplicative inverse.
...
was specifically design for that purpose. In turn, Elliptic Curves improve the performance of regulard DSA 
algorithm.
...
as they reqruire considerable storage and can be used only one-time as the name implies. 
...
To encrypt the message $\mu \in \{0, 1\}$ choose ${\bf r} \sim U(\{0, 1\}^m)$. Then we compute ${\bf u}={\bf rA}$ and 
...
that provides such services as authentication and link configuariont (\ie using Link Control Protocol (LCP)).

SSTP protocol is built on top of existing SSL. It allows to tunnel user traffic over protected 
channel, and yet look like normal HTTPS traffic to service providers.
...
agnostic hosts through a off-the-path black box that encrypts all the traffic and sends encapsulated in TCP and SSL
...
The major problem with the HIP-VPLS. Imagine that there are 
not 10's, but 1000's of sites and that all sites need to be 
combined into single network. First, of all there will be 
$O(n^2)$ pseudo-wires, which is for $1000$ PEs is somewhere 
around $1$M of routing entries. Second, HIP-VPLS provides 
signle broadcast domain. And so there is going to be 
chaos in the network which will be overwhelemed with broadcast
and multicast Ethernet frames. All this makes this type of
arrangement of network unacceptable in aformention scenarios.
...
forwarding the packets between the networks (sites). 
...
We will compare the two networks in proceeding section. And now it is worth
...
AES (with 256 bits key) algorithms. In our prototype implementation we 
have populated the routing tables manually, however, in practice the 
entries in practice this process should be automated using for example 
central controller. 
...
characteristics and limitation. In Table~\ref{tab:analysis} we 
