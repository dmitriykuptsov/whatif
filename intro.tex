...
problem is considered as hard as CVP on lattice.

Given a matrix $\bf{A} \sim U(Z^{nxm}_q)$, $\bf{s} \sim U(Z^{n}_q)$ and $\bf{e} \sim D_{Z^{m}, \sigma}$ 
sampled from discrete (clipped) Gaussian distribution with parameter $\sigma$. We require that, the probability
$P[e < q/4]$ is high (\ie $99.99\%$) to ensure correct decryption of the message and achieve required 
level of security. To ensure that $\bf{e}$ is always less than $q/4$ the values are drawn with resampling
or clipping. We require, that the parameter $\sigma = q/16$.
...
Once the parameters are generated, compute ${\bf Ax}+{\bf e} = {\bf b}$. Then, the public 
key is $({\bf A, b})$ and the private key is ${\bf s}$.

To encrypt the message $\mu \in \{0, 1\}$ choose ${\bf r} \sim U(\{0, 1\}^m)$. Then compute ${\bf u}={\bf rA}$ and 
$v={\bf rb}+\lfloor q/2 \rceil \mu$. The ciphertext is $({\bf u}, v)$.

To decrypt the message compute $v - \bf{u s}$, if the result is less than $q/4$ output $0$, if the result
