...
and decryption are performed on separate bits, one bit at a time. RC4 is an example 
...
$ed \equiv 1\ mod\ \phi(n)$ using modular multiplicative inverse (for that purpose extended Euclidean 
...
compute $v - \bf{u s}$: if the result is close to $0$ output $0$, otherwise, if the result
is close $q/2$ output $1$. %For decryption to work correctly, we require that the parameter $\sigma = q/(4m)$. 
...
it allows peers to create authenticated secure channels on the network layer (well, more precisely between layer 3 and layer 4), so all upper-layer protocols 
...
GRE on its own does not provide security at all and can be used together with IPsec to secure 
the traffic. PPTP does not provide strong security out of the box. PPTP uses 
...
port 4443 to remote web-server {\it example.com} on port 443:

\begin{verbatim}
ssh -L 192.168.1.1:4443:example.com:443 user@strangebit.io
\end{verbatim}

In this example, when the client types \url{https://192.168.1.1:4443/} in the browser window,
the traffic will be forwarded to the remote {\bf example.com} server through the SSH server \url{strangebit.io}.
...
throughout the several years that we have spent building VPLS system. We start 
...
running time in microseconds with the $95\%$ confidence intervals. For all configurations
we have performed $100$ measurements.
...
$58.9 \pm 0.52$ Mbit/s. Again for every setting we have perfomed $100$ 
measurement rounds.
...
In our testbed the central controller was responsible for collecting
liveness reports from HIP switches as well as provisioning 
the devices with the mesh configuration information, firewall rules 
and MAC-based ACLs. For that purpose, we developed a simple 
...
are the following: $19.7 \pm 0.06$ Mbit/s (again we have performed $100$ measurement rounds). This was expected, since the packet
is decrypted and encrypted, as well as HMAC is recalcutated at every hop on 
the path from source CE to destination CE. Perphaps, hop-by-hop encryption and authenication can
be done selectively with global secret key so that better performance can be achieved.
...
comprising provider equipment (PE).

The first characteristic is the size of the forwarding table on the PE elements. For 
L2-VPLS and HIP-VPLS it is equal to $O(n)$, where $n$ is the number of regular hosts in the
...
We foresee at least three possible research directions. The first one, is the implementation
of post-quantum cryptography algorithms in Host Identity Protocol. One can take a look
at the lattice-based cryptography as the alternative to RSA and DSA algorithms. The second
research direction can consider separation of control plane and data plane routines. Here 
one can consider implmenting IPSec data plane in pure C and leave the control plane as it is (that is
it can be based on Python). The data plane will be handling IPSec traffic and triggering base exachange wherever needed.
And finally, the third research direction can consider real life deployment of the 
architecture and more measurements in such real life environments.
...
production setup, and suggested implementing the HIP-VPLS in Rust or C language (or separate 
control and data plane, and suggested implementing data plane in C language)..
We then moved to the description of scalable L3-VPN and HIP-VPLS solutions. We concluded 
