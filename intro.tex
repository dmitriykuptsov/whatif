...
It is a layer $2.5$ solution that sits between data link layer and network layer.

In MPLS the packets are forwarded not using on MAC or IP addresses, but 
rather using lables, which are distributed by control protocol. Thus, when frame 
arrives at router the current lable is popped, new lable is added and the frame is
